...
wget -O clin-variant-etl.jar https://github.com/Ferlab-Ste-Justine/clin-variant-etl/releases/download/$VERSION/clin-variant-etl.jar

# copy + list S3, update profile name if necessary
echo "=== COPY TO S3 ($ENV) ==="
aws --profile cqgc-$ENV --endpoint https://s3.cqgc.hsj.rtss.qc.ca s3 cp clin-variant-etl.jar s3://cqgc-$ENV-app-datalake/jars/clin-variant-etl-$VERSION.jar
