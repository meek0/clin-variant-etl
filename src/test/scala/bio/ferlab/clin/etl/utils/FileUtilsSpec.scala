...
class FileUtilsSpec extends SparkSpec with WithTestConfig with CreateDatabasesBeforeAll with CleanUpBeforeAll {
  ...
  val clinicalDf = Seq(
    EnrichedClinical(`batch_id` = "B1", `patient_id` = "PA0001", `service_request_id` = "SRS0001", `aliquot_id` = "1", `specimen_id` = "1", `exomiser_urls` = Some(Set("s3a://file1.tsv", "s3a://file2.tsv", "s3a://file2b.tsv")), `covgene_urls` = Some(Set("s3a://file3covgene.csv")), `is_proband` = false),
  ...
  "fileUrls" should "return list of expected urls for a given batch id and file type" in {
    ...
    results should contain theSameElementsAs Seq(
      FileInfo(url = "s3a://file1.tsv", aliquot_id = "1", patient_id = "PA0001", specimen_id = "1", service_request_id = "SRS0001", is_proband = false),
      FileInfo(url = "s3a://file2.tsv", aliquot_id = "1", patient_id = "PA0001", specimen_id = "1", service_request_id = "SRS0001", is_proband = false),
      FileInfo(url = "s3a://file2b.tsv", aliquot_id = "1", patient_id = "PA0001", specimen_id = "1", service_request_id = "SRS0001", is_proband = false),
      FileInfo(url = "s3a://file3.tsv", aliquot_id = "2", patient_id = "PA0001", specimen_id = "2", service_request_id = "SRS0002", is_proband = true),
    )
