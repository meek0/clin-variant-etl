  ...
  it should "prepare input file for svclustering nextflow pipeline" in {
    val inputData: Map[String, DataFrame] = Map(
      enriched_clinical.id -> Seq(
        ...
        // Solo
        EnrichedClinical(aliquot_id = "11", is_proband = true, batch_id = "BAT2", analysis_service_request_id = "SRA2", father_aliquot_id = None, mother_aliquot_id = None, cnv_vcf_urls = Some(Set("s3a://11.vcf"))),

        // Duo with no CNVs: Should not be included
        EnrichedClinical(aliquot_id = "111", is_proband = true, batch_id = "BAT3", analysis_service_request_id = "SRA3", father_aliquot_id = None, mother_aliquot_id = Some("222"), cnv_vcf_urls = None),
        EnrichedClinical(aliquot_id = "222", is_proband = false, batch_id = "BAT3", analysis_service_request_id = "SRA3", father_aliquot_id = None, mother_aliquot_id = None, cnv_vcf_urls = None),
      ).toDF()
    )
