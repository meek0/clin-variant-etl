...
object FileUtils {
   ...
   */
  def filesUrl(batchId: String, dataType: String, format: String)(implicit spark: SparkSession, c: Configuration): Set[String] = {
  ...
  def filesUrlFromDF(tasks: DataFrame, documentReferences: DataFrame, batchId: String, dataType: String, format: String)(implicit spark: SparkSession): Set[String] = {
    import spark.implicits._
    val documentIdsFromTasks = tasks
      .where(col("experiment.name") === batchId)
      .select(explode(col("documents")) as "document")
      .where(col("document.document_type") === dataType)
      .select(col("document.id") as "id")
    val documents = documentReferences
      .where(col("type") === dataType)
      .select(col("id"), explode(col("contents")) as "content")
      .where(col("content.format") === format)
      .join(documentIdsFromTasks, Seq("id"), "inner")
      .select(col("content.s3_url") as "url")
    documents.as[String].collect().toSet
  }
