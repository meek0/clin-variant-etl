...
object EtlConfiguration extends App {
  ...
  val sources =
    List(
      ...
      DatasetConf("raw_exomiser_cnv"          , clin_nextflow, "/cnv_post_processing/output/exomiser/{{ANALYSIS_ID}}.exomiser.variants.tsv" , CSV , OverWrite, readoptions = tsv_with_headers),
      ...
      DatasetConf("raw_person"                , clin_datalake, "/raw/landing/fhir/Person"                                     , JSON, OverWrite),
      ...
      DatasetConf("normalized_person"              , clin_datalake, "/normalized/fhir/Person"            , DELTA  , OverWrite   , TableConf("clin", "fhir_person")),
      ...
      DatasetConf("nextflow_svclustering_parental_origin"       , clin_datalake, "/nextflow/svclustering_parental_origin"                                           , DELTA, OverWritePartition, partitionby = List("analysis_id"), table = Some(TableConf("clin", "nextflow_svclustering_parental_origin")), keys = List("name", "sequencing_id"), repartition = Some(RepartitionByColumns(Seq("analysis_id"), n = Some(1)))),

      //clinical normalized
      DatasetConf("normalized_snv"                            , clin_datalake, "/normalized/snv"                    , DELTA  , OverWritePartition, partitionby = List("analysis_id"), table = Some(TableConf("clin", "normalized_snv"))),
      ...
      DatasetConf("normalized_cnv"                            , clin_datalake, "/normalized/cnv"                    , DELTA  , OverWritePartition, partitionby = List("analysis_id"), table = Some(TableConf("clin", "normalized_cnv"))),
      DatasetConf("normalized_cnv_somatic_tumor_only"         , clin_datalake, "/normalized/cnv_somatic_tumor_only" , DELTA  , OverWritePartition, partitionby = List("analysis_id"), table = Some(TableConf("clin", "normalized_cnv_somatic_tumor_only"))),
      DatasetConf("normalized_variants"            , clin_datalake, "/normalized/variants"               , DELTA  , OverWritePartitionDynamic, partitionby = List("analysis_id", "bioinfo_analysis_code"), table = Some(TableConf("clin", "normalized_variants")), repartition = Some(RepartitionByColumns(columnNames = Seq("chromosome"), n=Some(1), sortColumns = Seq("start")))),
      ...
      DatasetConf("normalized_exomiser"            , clin_datalake, "/normalized/exomiser"               , DELTA  , OverWritePartition, partitionby = List("analysis_id")           , table = Some(TableConf("clin", "normalized_exomiser"))),
      DatasetConf("normalized_exomiser_cnv"        , clin_datalake, "/normalized/exomiser_cnv"           , DELTA  , OverWritePartition, partitionby = List("analysis_id")           , table = Some(TableConf("clin", "normalized_exomiser_cnv"))),
      DatasetConf("normalized_coverage_by_gene"    , clin_datalake, "/normalized/coverage_by_gene"       , DELTA  , OverWritePartitionDynamic, partitionby = List("analysis_id", "bioinfo_analysis_code")           , table = Some(TableConf("clin", "normalized_coverage_by_gene"))),
      ...
      DatasetConf("enriched_cnv"                   , clin_datalake, "/enriched/cnv"                      , DELTA  , OverWritePartition, partitionby = List("analysis_id")                       , table = Some(TableConf("clin", "cnv"))              , keys = List("hash"), repartition = Some(RepartitionByColumns(Seq("analysis_id"), n = Some(1), sortColumns = Seq("name")))),
