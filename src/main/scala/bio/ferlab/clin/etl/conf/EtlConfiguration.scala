...
object EtlConfiguration extends App {
  ...
  val sources =
    List(
      ...
      DatasetConf("enriched_cnv"                   , clin_datalake, "/enriched/cnv"                      , DELTA  , OverWritePartition, partitionby = List("service_request_id")                       , table = Some(TableConf("clin", "cnv"))              , keys = List("name", "service_request_id"), repartition = Some(RepartitionByColumns(Seq("service_request_id"), n = Some(1), sortColumns = Seq("name")))),
