...
object EtlConfiguration extends App {
  ...
  val sources =
    List(
      ...
      DatasetConf("normalized_variants"            , clin_datalake, "/normalized/variants"               , DELTA  , OverWritePartitionDynamic, partitionby = List("analysis_id", "bioinfo_analysis_code"), table = Some(TableConf("clin", "normalized_variants")), repartition = Some(FixedRepartition(1))),
      ...
      DatasetConf("enriched_cnv"                   , clin_datalake, "/enriched/cnv"                      , DELTA  , OverWritePartitionDynamic, partitionby = List("analysis_id")                       , table = Some(TableConf("clin", "cnv"))              , keys = List("hash"), repartition = Some(RepartitionByColumns(Seq("analysis_id"), n = Some(1), sortColumns = Seq("name")))),
