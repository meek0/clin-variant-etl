...
object FiltersFrequencyExtra extends TestingApp {
  run { spark =>
    ...
    val df_NorSNV = normalized_snv
      .join(
        normalized_snv.filter($"gq" >= 20 && col("filters") === Array("PASS") && $"ad_alt" >= 3 && $"alternate" =!= "*"),
        Seq("chromosome", "start", "reference", "alternate"), "left_anti"
      )
    ...
    handleErrors(
      shouldBeEmpty(
        df_NorSNV.join(df_NorVar, Seq("chromosome", "start", "reference", "alternate"), "inner")
      )
    )
  }
}
