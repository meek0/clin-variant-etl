...
object TestingApp {
  ...
  val includeFilter_somatic: Column = col("ad_alt") >= 2 && col("alternate") =!= "*"
  ...
  val pc_somatic: Column = sum(when(array_contains(col("calls"), 1) and includeFilter_somatic, 1).otherwise(0)) as "expected_pc"
