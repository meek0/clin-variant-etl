...
trait TestingApp extends App {
  ...
  lazy val nextflow_svclustering_germline: DataFrame = spark.table("nextflow_svclustering_germline")
  lazy val nextflow_svclustering_somatic: DataFrame = spark.table("nextflow_svclustering_somatic")
...
object TestingApp {
  ...
  def TestDfContainsAllVarFromBatch(df: DataFrame, b:String, adAltFilter:Number, database: String)(implicit spark: SparkSession): Option[String] = {
    ...
    .where(!$"aliquot_id".isin("21375", "21388", "21389", "23276", "23277", "RetraitProb", "RetraitMth", "RetraitFth", "21771", "21772")) // Retrait de patients du metadata mais pas du VCF
  ...
  val pc_somatic: Column = sum(when(array_contains(col("calls"), 1) and includeFilter, 1).otherwise(0)) as "expected_pc"
